...
class Pro_Upgrade {
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget(
			'jltelcl_dashboard_widget',                          // Widget slug.
			esc_html__( 'content-locker-for-elementor News & Updates', 'content-locker-for-elementor' ), // Title.
		...
		$example_widget_backup = array( 'jltelcl_dashboard_widget' => $default_dashboard['jltelcl_dashboard_widget'] );
		unset( $default_dashboard['jltelcl_dashboard_widget'] );
	...
	public function dashboard_widget_render() {
		...
		if ( wp_validate_boolean( $this->get_content( 'is_campaign' ) ) ) { ?>
			...
			<div class="-dashboard-promo" style="---popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				<a target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
					<img src="<?php echo esc_url( $this->get_content( 'image_url' ) ); ?>" alt="content-locker-for-elementor Promo Image" style="width: 100%; height: auto;">
				</a>
		...
		if ( ! empty( $maxitems ) ) {
			...
			foreach ( $rss_items as $item ) {
				...
				$item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'content-locker-for-elementor' );
				...
				<li class="-overview__blog">
                    <a href="https://jeweltheme.com/blog" target="_blank">
                        ...
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'content-locker-for-elementor' ); ?>
                        </span>
                        <span aria-hidden="true" class="dashicons dashicons-external"></span>
                    </a>
                </li>
                <li class="-overview__help">
	...
	public static function get_data() {
		return get_option( 'jltelcl_sheet_promo_data' );
	}
	...
	public function get_data_hash() {
		return get_option( 'jltelcl_sheet_promo_data_hash' );
	}
	...
	public function register_sync_hook() {
		$hook_action = 'jltelcl_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook( JLTELCL_FILE, array( $this, 'clear_register_sync_hook' ) );
	}
	...
	public function clear_register_sync_hook() {
		wp_clear_scheduled_hook( 'jltelcl_sheet_promo_data_remote_sync' );
	}
	...
	public function sheet_data_remote_sync() {
		...
		if ( $force || $sheet_hash_data !== $sheet_data_hash ) {
			update_option( 'jltelcl_sheet_promo_data', $remote_data );
			update_option( 'jltelcl_sheet_promo_data_hash', $sheet_data_hash );
			do_action( 'jltelcl_sheet_promo_data_reset' );
		}
	}
	...
	public function get_sheet_promo_remote_data() {
		...
		$plugin_slug = Helper::jltelcl_slug_cleanup();
