...
use JLTELCL\Inc\Classes\Notifications\Model\Notice;
...
if ( ! class_exists( 'What_We_Collect' ) ) {
	...
	class What_We_Collect extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_jltelcl_allow_collect', array( $this, 'jltelcl_allow_collect' ) );
		}
		...
		public function set_title() {
			printf(
				...
				esc_html__( 'content-locker-for-elementor', 'content-locker-for-elementor' )
			);
		}
		...
		public function what_we_collect_link() {
			echo '<a href="!#" class="-wwc-link">' . esc_html__( 'what we collect', 'content-locker-for-elementor' ) . '</a>';
		}
		...
		public function notice_content() { ?>
			<div class="-notice-review-box -wwc">
				...
				<?php
				echo sprintf(
					__( 'Want to help make <strong>%1$s</strong> even more awesome? Allow %1$s to collect non-sensitive diagnostic data and usage information.', 'content-locker-for-elementor' ),
					__( 'content-locker-for-elementor', 'content-locker-for-elementor' )
				);
				?>
		...
		public function footer_content() {
			...
			<a class="button button-primary allow-button" href="<?php echo esc_url( $this->plugin_rate_url() ); ?>" rel="nofollow" target="_blank">
				<?php echo esc_html__( 'Allow', 'content-locker-for-elementor' ); ?>
			</a>
			<a class="button button-secondary button-reject -notice-dismiss" href="#" rel="nofollow">
				<?php echo esc_html__( 'No Thanks', 'content-locker-for-elementor' ); ?>
			</a>
			<?php
		}
			...
			<script>
				...
				jQuery('body').on('click', '.-notice-what_we_collect .allow-button', function(e) {
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'jltelcl_allow_collect',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jltelcl_allow_collect_nonce' ) ); ?>',
							}
						})
						.done(function(response) {
							...
							let subsTimeout = setTimeout(function() {
								jltelcl_notice_action(null, noticeWrapper.children(), 'disable');
