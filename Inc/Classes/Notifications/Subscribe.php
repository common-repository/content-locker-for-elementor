...
if ( ! class_exists( 'Subscribe' ) ) {
	...
	class Subscribe extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_jltelcl_subscribe', array( $this, 'jltelcl_subscribe' ) );
		}
		...
		public function notice_content() {
			...
			<div class="notice notice- is-dismissible notice-plugin-review notice-<?php echo esc_attr( $this->color ); ?> -notice-<?php echo esc_attr( $this->get_id() ); ?>">
				...
				<img width="70" height="70" src="<?php echo esc_url( JLTELCL_IMAGES . '/logo.png' ); ?>" alt="<?php esc_attr_e( 'Logo', 'content-locker-for-elementor' ); ?>">
				<div class="-subscribe-content">
					...
					<form style="display:flex">
						...
						<button type="submit" class="button button-primary -subscribe-btn"><?php esc_html_e( 'Get Discount', 'content-locker-for-elementor' ); ?></button>
					</form>
				</div>
			</div>
			<?php
		...
		public function plugin_rate_url() {
			return 'https://wordpress.org/plugins/' . JLTELCL_SLUG;
		}
		...
		public function footer_content() {
			...
			<a class="button button-primary rate-plugin-button" href="<?php echo esc_url( $this->plugin_rate_url() ); ?>" rel="nofollow" target="_blank">
				<?php echo esc_html__( 'Rate Now', 'content-locker-for-elementor' ); ?>
			</a>
			<a class="button notice-review-btn review-later -notice-dismiss" href="#" rel="nofollow">
				<?php echo esc_html__( 'Later', 'content-locker-for-elementor' ); ?>
			</a>
			<a class="button notice-review-btn review-done -notice-disable" href="#" rel="nofollow">
				<?php echo esc_html__( 'I already did', 'content-locker-for-elementor' ); ?>
			</a>
			<?php
		}
			...
			<script>
				jQuery(document).on('submit', '.-notice-subscribe .-subscribe-content form', function(e) {
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'jltelcl_subscribe',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jltelcl_subscribe_nonce' ) ); ?>',
								name: name,
								email: email,
							}
						})
						.done(function(response) {
							...
							let subsTimeout = setTimeout(function() {
								jltelcl_notice_action(null, form, 'disable');
